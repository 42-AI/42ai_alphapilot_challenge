...
from ssd.prior_util import PriorUtil
...
if __name__ == "__main__":
    ...
    train_name = "Test_{}_{}_{}".format(model, type, datadir[0].split('/')[-1])
    ...
    model = get_model_by_name(model, cpu_fix=True)

    train.setModel(model, weightpath=weights, multi_gpu=True)
    train.setLoss(FocalLoss())
    train.setOptimizer(Adam(lr=1e-4, beta_1=0.9, beta_2=0.999, epsilon=0.001, decay=0.0))
